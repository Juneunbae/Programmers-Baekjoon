...
def solution(players, callings):
    ...
    for c in callings :
        cur_rank = name_to_rank[c]
        if (cur_rank == 0) :
            continue
            
        front_name = rank_to_name[cur_rank - 1]

        rank_to_name[cur_rank - 1], rank_to_name[cur_rank] = rank_to_name[cur_rank], rank_to_name[cur_rank - 1]

        name_to_rank[c] -= 1
        name_to_rank[front_name] += 1
