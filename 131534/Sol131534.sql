...
'''
SELECT
    YEAR(O.SALES_DATE) AS YEAR, MONTH(O.SALES_DATE) AS MONTH, COUNT(DISTINCT(I.USER_ID)) AS PURCHASED_USERS,
    ROUND(COUNT(DISTINCT(I.USER_ID)) / (SELECT COUNT(USER_ID) FROM USER_INFO WHERE YEAR(JOINED) = 2021), 1) AS PUCHASED_RATIO
FROM USER_INFO I
