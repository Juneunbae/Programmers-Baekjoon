...
public class Sol0606 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        int n = sc.nextInt();
        int[] changeArr = new int[n];
        for(int i = 0; i < n; i++) changeArr[i] = sc.nextInt();

        int[] originalArr = changeArr.clone();
        Arrays.sort(originalArr);
        
        for(int i = 0; i < n; i++) {
            if (originalArr[i] != changeArr[i]) System.out.print(i + 1 + " ");
        }
    }
}
