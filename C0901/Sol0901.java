/**
    ...
    각 선수의 흰돌능력치가 모두 다르고, 검은돌 능력치도 모두 다릅니다. 
...
class Sol0901 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) humans.add(new Human(sc.nextInt(), sc.nextInt()));
        Collections.sort(humans);

        int answer = 1;
        int baseWeight = humans.get(0).weight;

        for (int i = 1; i < n; i++) {
            if (humans.get(i).weight > baseWeight) {
                answer++;
                baseWeight = humans.get(i).weight;
            }
        }
